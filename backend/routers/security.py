...
@keys_router.post("", status_code=status.HTTP_201_CREATED, summary=Summaries.KEY_GENERATE)
...
@keys_router.get("/{id}", summary=Summaries.KEY_GET)
async def get_key(id: str) -> Dict[str, Any]:
    return {"id": id}


@policies_router.post("", status_code=status.HTTP_201_CREATED, summary=Summaries.POLICY_CREATE)
...
@policies_router.get("/{id}", summary=Summaries.POLICY_GET)
async def get_policy(id: str) -> Dict[str, Any]:
    return {"id": id}


@policies_router.put("/{id}", summary=Summaries.POLICY_UPDATE)
...
@policies_router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT, summary=Summaries.POLICY_DELETE)
