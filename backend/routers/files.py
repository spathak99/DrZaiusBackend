...
@router.get("/{id}", summary=Summaries.FILE_GET)
async def get_file(id: str) -> Dict[str, Any]:
    return service.get(id)


@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT, summary=Summaries.FILE_DELETE)
async def delete_file(id: str) -> None:
    service.delete(id)
    return


@router.post("/{fileId}/embeddings", status_code=status.HTTP_201_CREATED, summary=Summaries.FILE_EMBEDDINGS_CREATE)
...
@router.get("/{fileId}/embeddings", summary=Summaries.FILE_EMBEDDINGS_GET)
async def get_file_embeddings(fileId: str) -> Dict[str, Any]:
    return {"fileId": fileId, "embeddings": []}


@router.get("/{id}/download", summary=Summaries.FILE_DOWNLOAD)
async def download_file(id: str) -> Dict[str, Any]:
    return {"id": id, "download": "link"}


@router.get("/{fileId}/access", summary=Summaries.FILE_ACCESS_LIST)
async def list_file_access(fileId: str) -> Dict[str, Any]:
    return {"fileId": fileId, "items": service.list_access(fileId)}


@router.post("/{fileId}/access", status_code=status.HTTP_201_CREATED, summary=Summaries.FILE_ACCESS_GRANT)
...
@router.put("/{fileId}/access/{caregiverId}", summary=Summaries.FILE_ACCESS_UPDATE)
...
@router.delete("/{fileId}/access/{caregiverId}", status_code=status.HTTP_204_NO_CONTENT, summary=Summaries.FILE_ACCESS_REVOKE)
