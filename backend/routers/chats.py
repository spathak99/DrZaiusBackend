...
@router.post("", status_code=status.HTTP_201_CREATED, summary=Summaries.CREATE_CHAT)
async def create_chat(payload: ChatCreate = Body(default=None)) -> Dict[str, Any]:
    return service.create_chat(payload.model_dump())


@router.put("/{id}", summary=Summaries.UPDATE_CHAT)
...
@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT, summary=Summaries.DELETE_CHAT)
async def delete_chat(id: str) -> None:
    service.delete_chat(id)
    return


@router.post("/{chatId}/embeddings", status_code=status.HTTP_201_CREATED, summary=Summaries.CHAT_EMBEDDINGS_CREATE)
...
@router.get("/{chatId}/embeddings", summary=Summaries.CHAT_EMBEDDINGS_GET)
